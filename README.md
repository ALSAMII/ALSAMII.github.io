...
**See [ADDING-A-BOOK.md](ADDING-A-BOOK.md).** Six steps, in order, with
...
## The two generated things

Neither is written by hand. Both read `stories.js`:

```
node build-feeds.js         → feed.xml, sitemap.xml
node build-share-pages.js   → b/
```

Run both whenever a book is added or changed, and commit what they
produce. `feed.xml` and `sitemap.xml` are how search engines and feed
readers see a site whose list is built in JavaScript. The `b/` folder is
how a shared link shows the right cover — see ADDING-A-BOOK.md for why
that can't be done with a `#` address.
