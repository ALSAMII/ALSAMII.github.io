...
def measure(pdf):
    ...
    for page in pdf.pages[:24]:
        ...
        for line in page.extract_text_lines():
            ...
            prev = line["top"]
    if not sizes:
        return 10.0, 60, 16, 60 + INDENT_PT
    body = max(sizes, key=sizes.get)
    left = max(lefts, key=lefts.get)
    # The step a paragraph is indented by, measured rather than assumed.
    # Anything set further right than this is a deeper level again —
    # in these books, verse. A book that has no such level simply
    # reports its own paragraph indent and nothing can clear it.
    deeper = {x: n for x, n in ins.items() if x > left + 1}
    indent = max(deeper, key=deeper.get) if deeper else left + INDENT_PT
...
def extract(path):
    ...
    with pdfplumber.open(path) as pdf:
        body, left, lead, indent = measure(pdf)
        ...
        for page in pdf.pages:
            ...
            for line in page.extract_text_lines():
                ...
                # Verse that announces itself by setting rather than by
                # a word. A whole line in italic, set further right than
                # this book's own paragraph indent, is not prose: an
                # italic run inside a paragraph never begins a line at a
                # deeper level than the paragraph itself. A book with no
                # level deeper than its indent can never satisfy this.
                if (line["x0"] >= indent + INDENT_PT and len(text) < 70
                        and all("Italic" in c["fontname"]
                                or "Oblique" in c["fontname"]
                                for c in line["chars"])):
                    flush()
    ...
    for b in blocks:
        ...
        if skipping:
            ...
            if (b["t"] == "p" and len(plain.split()) > 25
                    and len(re.findall(r"(?<![\w.])\d{1,3}(?![\w.])",
                                       plain)) < 4):
                skipping = False
            else:
                continue
